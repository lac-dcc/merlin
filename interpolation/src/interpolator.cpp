...
Interpolator::Interpolator(std::vector<double> x, std::vector<double> y, std::vector<double> z, std::vector<double> fxyz) {
  this->interpType = InterpolationType::ThreeVar;
  this->X = x;
  this->Y = y;
  this->Z = z;
  this->F = fxyz;
...
std::string Interpolator::interpolate() {
  switch(this->interpType) {
    case InterpolationType::OneVar:
      return this->newtonDividedDifference();
      break;
    case InterpolationType::TwoVar:
      return this->LeastSquares2var();
      break;
    case InterpolationType::ThreeVar:
      return this->LeastSquares3var();
      break;
    default:
      return "There's no inteporlation method avaible for this constraints.";
  }
...
std::string Interpolator::LeastSquares2var() {
...
std::string Interpolator::LeastSquares3var() {
  Eigen::MatrixXd A(this->numSample, this->numSample);
  Eigen::VectorXd b(this->numSample);
  Eigen::VectorXd answer;
