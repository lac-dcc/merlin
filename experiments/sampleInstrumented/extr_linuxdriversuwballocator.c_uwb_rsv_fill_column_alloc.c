...
struct TYPE_2__ {
  int start_col;
  int safe_mas_per_col;
  int unsafe_mas_per_col;
  scalar_t__ interval;
};
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_ai0 = 65025;
    struct uwb_rsv_alloc_info *ai = (struct uwb_rsv_alloc_info *)malloc(
        _len_ai0 * sizeof(struct uwb_rsv_alloc_info));
    for (int _i0 = 0; _i0 < _len_ai0; _i0++) {
      int _len_ai__i0__bm0 = 1;
      ai[_i0].bm =
          (unsigned char *)malloc(_len_ai__i0__bm0 * sizeof(unsigned char));
      for (int _j0 = 0; _j0 < _len_ai__i0__bm0; _j0++) {
        ai[_i0].bm[_j0] = ((-2 * (next_i() % 2)) + 1) * next_i();
      }
      int _len_ai__i0__ci0 = 1;
      ai[_i0].ci = (struct uwb_rsv_col_info *)malloc(
          _len_ai__i0__ci0 * sizeof(struct uwb_rsv_col_info));
      for (int _j0 = 0; _j0 < _len_ai__i0__ci0; _j0++) {
        ai[_i0].ci->csi.start_col = ((-2 * (next_i() % 2)) + 1) * next_i();
        ai[_i0].ci->csi.safe_mas_per_col =
            ((-2 * (next_i() % 2)) + 1) * next_i();
        ai[_i0].ci->csi.unsafe_mas_per_col =
            ((-2 * (next_i() % 2)) + 1) * next_i();
