...
__attribute__((used)) static void construct_mic_iv(u8 *mic_iv, int qc_exists,
                                                   int a4_exists, u8 *mpdu,
                                                   uint payload_length,
                                                   u8 *pn_vector) {
  unsigned counterconstruct_mic_iv1 = 0;
  unsigned counterconstruct_mic_iv0 = 0;
  ...
  for (i = 2; i < 8; i++) {
    counterconstruct_mic_iv0++;
    mic_iv[i] = mpdu[i + 8]; /* mic_iv[2:7] = A2[0:5] = mpdu[10:15] */
  }
  for (i = 8; i < 14; i++) {
    counterconstruct_mic_iv1++;
