...
process_compressed_8x8_brush_data(uint8 *in, uint8 *out, int Bpp) {
  unsigned counterprocess_compressed_8x8_brush_data0 = 0;
  unsigned counterprocess_compressed_8x8_brush_data1 = 0;
  unsigned counterprocess_compressed_8x8_brush_data3 = 0;
  unsigned counterprocess_compressed_8x8_brush_data2 = 0;
  ...
  /* read it bottom up */
  for (y = 7; y >= 0; y--) {
    counterprocess_compressed_8x8_brush_data0++;
    /* 2 bytes per row */
    x = 0;
    for (do2 = 0; do2 < 2; do2++) {
      counterprocess_compressed_8x8_brush_data1++;
      /* 4 pixels per byte */
      shift = 6;
      while (shift >= 0) {
        counterprocess_compressed_8x8_brush_data2++;
        pal_index = (in[in_index] >> shift) & 3;
        /* size of palette entries depends on Bpp */
        for (i = 0; i < Bpp; i++) {
          counterprocess_compressed_8x8_brush_data3++;
  ...
  printf("%d\n", counterprocess_compressed_8x8_brush_data0);
  printf("%d\n", counterprocess_compressed_8x8_brush_data1);
  printf("%d %d\n", tempBpp, counterprocess_compressed_8x8_brush_data3);
  printf("%d\n", counterprocess_compressed_8x8_brush_data2);
}
