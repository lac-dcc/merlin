...
ContextualShape_Control(HDC hdc, ScriptCache *psc, SCRIPT_ANALYSIS *psa,
                        WCHAR *pwcChars, INT cChars, WORD *pwOutGlyphs,
                        INT *pcGlyphs, INT cMaxGlyphs, WORD *pwLogClust) {
  unsigned counterContextualShape_Control0 = 0;

  INT tempcChars = cChars;

  int i;
  for (i = 0; i < cChars; i++) {
    counterContextualShape_Control0++;
    switch (pwcChars[i]) {
    case 0x000A:
    case 0x000D:
  ...
  printf("at line 78 :");
  printf(" cChars\n");
  printf("%d %d\n", tempcChars, counterContextualShape_Control0);
}
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_psc0 = 65025;
    struct TYPE_5__ *psc =
        (struct TYPE_5__ *)malloc(_len_psc0 * sizeof(struct TYPE_5__));
    ...
    ContextualShape_Control(hdc, psc, psa, pwcChars, cChars, pwOutGlyphs,
                            pcGlyphs, cMaxGlyphs, pwLogClust);
    free(psc);
    free(psa);
    free(pwcChars);
    free(pwOutGlyphs);
