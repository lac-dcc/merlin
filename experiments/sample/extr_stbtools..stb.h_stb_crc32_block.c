...
stb_uint stb_crc32_block(stb_uint crc, unsigned char *buffer, stb_uint len) {
  unsigned counterstb_crc32_block0 = 0;
  unsigned counterstb_crc32_block2 = 0;
  unsigned counterstb_crc32_block1 = 0;

  stb_uint templen = len;
  ...
  if (crc_table[1] == 0) {
    for (i = 0; i < 256; i++) {
      counterstb_crc32_block0++;
      for (s = i, j = 0; j < 8; ++j) {
        counterstb_crc32_block1++;
  ...
  printf("\n");
  printf("%d\n", counterstb_crc32_block0);
  printf("%d %d\n", templen, counterstb_crc32_block2);
  printf("%d\n", counterstb_crc32_block1);
  return ~crc;
}
