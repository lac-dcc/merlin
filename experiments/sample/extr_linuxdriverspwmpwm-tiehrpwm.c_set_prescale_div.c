...
__attribute__((used)) static int set_prescale_div(unsigned long rqst_prescaler,
                                                  u16 *prescale_div,
                                                  u16 *tb_clk_div) {
  unsigned counterset_prescale_div0 = 0;
  unsigned counterset_prescale_div1 = 0;

  unsigned int clkdiv, hspclkdiv;

  for (clkdiv = 0; clkdiv <= CLKDIV_MAX; clkdiv++) {
    counterset_prescale_div0++;
    for (hspclkdiv = 0; hspclkdiv <= HSPCLKDIV_MAX; hspclkdiv++) {
      counterset_prescale_div1++;
      /*
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    unsigned long *prescale_div =
        (unsigned long *)malloc(_len_prescale_div0 * sizeof(unsigned long));
    for (int _i0 = 0; _i0 < _len_prescale_div0; _i0++) {
      prescale_div[_i0] = ((-2 * (next_i() % 2)) + 1) * next_i();
    }
    int _len_tb_clk_div0 = 1;
    unsigned long *tb_clk_div =
        (unsigned long *)malloc(_len_tb_clk_div0 * sizeof(unsigned long));
