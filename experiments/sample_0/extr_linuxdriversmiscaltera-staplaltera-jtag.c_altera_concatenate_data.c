...
altera_concatenate_data(u8 *buffer, u8 *preamble_data, u32 preamble_count,
                        u8 *target_data, u32 start_index, u32 target_count,
                        u8 *postamble_data, u32 postamble_count)
/*
 * Copies preamble data, target data, and postamble data
 * into one buffer for IR or DR scans.
 */
{
  unsigned counteraltera_concatenate_data1 = 0;
  unsigned counteraltera_concatenate_data0 = 0;
  unsigned counteraltera_concatenate_data2 = 0;

  u32 temppreamble_count = preamble_count;
  u32 temptarget_count = target_count;
  u32 temppostamble_count = postamble_count;

  u32 i, j, k;

  for (i = 0L; i < preamble_count; ++i) {
    counteraltera_concatenate_data0++;
  ...
  for (; i < k; ++i, ++j) {
    counteraltera_concatenate_data1++;
  ...
  for (; i < k; ++i, ++j) {
    counteraltera_concatenate_data2++;
  ...
  printf("3\n");
  printf("at line 81 :");
  printf(" preamble_count target_count postamble_count\n");
  printf("at line 71 :");
  printf(" preamble_count\n");
  printf("at line 91 :");
  printf(" preamble_count target_count postamble_count\n");
  printf("%ld %ld %ld %d\n", temppreamble_count, temptarget_count,
         temppostamble_count, counteraltera_concatenate_data1);
  printf("%ld %d\n", temppreamble_count, counteraltera_concatenate_data0);
  printf("%ld %ld %ld %d\n", temppreamble_count, temptarget_count,
         temppostamble_count, counteraltera_concatenate_data2);
}
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    altera_concatenate_data(buffer, preamble_data, preamble_count, target_data,
                            start_index, target_count, postamble_data,
                            postamble_count);
    free(buffer);
    free(preamble_data);
    free(target_data);
